...
export default function ScrollSlider({ destinations }: Props) {
  ...
  useEffect(() => {
    ...
    // This component uses GSAP which depends on browser globals.
    // Load it dynamically to avoid running it during server-side rendering.
    (async () => {
      const [{ default: gsapLib }, { SplitText: SplitTextLib }] =
        await Promise.all([import("gsap"), import("gsap/SplitText")]);
      gsap = gsapLib;
      SplitText = SplitTextLib;
      ...
      function handleScrollDown() {
        ...
        const newBackIndex =
          (frontSlideIndex.current + 4) % destinations.length;
